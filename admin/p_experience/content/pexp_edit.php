...
if (isset($_POST["u_panda"])) {
    ...
    if ($_FILES['acctactivation']['name'] != "") {
        ...
        $folder = '../../../people_management/assets/img/' . $filename;
        $folder_db = '/Employee-Portal-v2/people_management/assets/img/' . $filename;
    ...
    for ($i = 1; $i <= $len; $i++) {
        if (!empty($_POST['e_name' . $i] && $_POST['e_team' . $i] && $_POST['e_date' . $i] && $_POST['e_content' . $i])) {
            ...
            if ($_FILES['e_img'.$i]['name'] != "") {
                ...
                $folder_db = '/Employee-Portal-v2/people_operations/people_experience/assets/files/' . $filename;
